...
class BlockChain(object):
  ...
  def create_block(self, nonce, previous_hash):
    block = utils.sorted_doct_by_key({
      'timestamp': time.time(),
  ...
  def add_transaction(self, sender_blockchain_address, recipient_blockchain_address, value, sender_public_key=None, signature=None):
    transaction = utils.sorted_doct_by_key({
    ...
    if self.verify_transaction_signature(sender_public_key, signature, transaction):
      ...
      #   logger.error({'action':'add_transaction','erorr':'no_value'})
      #   return False
  ...
  def valid_proof(self, transactions, previous_hash, nonce, difficuluty=MINING_DIFFICULTY):
    guess_block = utils.sorted_doct_by_key({
      "transactions":transactions,
      "previous_hash":previous_hash,
      "nonce":nonce
    })
    guess_hash = self.hash(guess_block)
    return guess_hash[:difficuluty] == "0"*difficuluty
